// <auto-generated> This file has been auto generated by EF Core Power Tools. </auto-generated>
#nullable disable
namespace Cidco.Middleware.Domain.Entities;

public partial class AapleUserTb
{
    public long Tid { get; set; }

    public string Trackid { get; set; }

    public string Userid { get; set; }

    public string UsertypeName { get; set; }

    public string UserName { get; set; }

    public string Password { get; set; }

    public string PasswordChanged { get; set; }

    public string EmailId { get; set; }

    public string Mobileno { get; set; }

    public string Fullname { get; set; }

    public string FullnameMr { get; set; }

    public string Occupation { get; set; }

    public string Age { get; set; }

    public string Gender { get; set; }

    public string Dob { get; set; }

    public string Uidno { get; set; }

    public string Panno { get; set; }

    public string Pincode { get; set; }

    public string Divisionid { get; set; }

    public string Districtid { get; set; }

    public string Talukaid { get; set; }

    public string Villageid { get; set; }

    public string RegistrationType { get; set; }

    public string DistrictPrefix { get; set; }

    public string ServiceId { get; set; }

    public string ApplicationId { get; set; }

    public string ApplicationDate { get; set; }

    public string UsrTimeStamp { get; set; }

    public string UsrSession { get; set; }

    public string ClientCheckSumValue { get; set; }

    public string StrServiceCookie { get; set; }

    public string PayStatus { get; set; }

    public string PayDate { get; set; }

    public string Digitalstatus { get; set; }

    public string DigitalDate { get; set; }

    public string ServiceDays { get; set; }

    public string ServiceDate { get; set; }

    public string Amount { get; set; }

    public string RequestFlag { get; set; }

    public string Appstatus { get; set; }

    public string Remark { get; set; }

    public DateTime? Entrydate { get; set; }

    public string RejectReason { get; set; }

    public DateTime? ActualServiceDate { get; set; }
}